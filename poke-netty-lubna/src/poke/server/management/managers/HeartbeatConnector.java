...
public class HeartbeatConnector extends Thread {
	...
	public void run() {
		...
		int count = 0;
		//ElectionManager Emanager = new ElectionManager();
				...
				for (HeartMonitor hb : monitors) {
					if (!hb.isConnected()) {
						try {
							...
							//// logger.info("************Heartbeat jsut sent");
							count++;
							//// logger.info("************Heartbeat count" + count);
							if(count >= 4)
							{
							//	// logger.info("************now u can call for elections******");
								
							 
							ElectionManager.getInstance().sendElectionMsg(hb.getNodeId(),"test", VoteAction.ELECTION);
							}
							//
						} catch (Exception ie) {
							// do nothing
